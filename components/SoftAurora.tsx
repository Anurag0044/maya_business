...
void main() {
  ...
  if (uLightMode > 0.5) {
    // Ultra-clean light mode rendering: delicate luminous pastel veil
    ...
    chroma = pow(clamp(chroma / max(peak, 0.0001), 0.0, 1.0), vec3(1.05));

    // Transparent alpha so it seamlessly blends onto light interfaces
    float alphaLight = clamp((weight1 + weight2) * uBrightness * 0.85, 0.0, 0.55);
    gl_FragColor = vec4(chroma, alphaLight);
  } else {
    // Executive dark mode rendering: deep obsidian with luminous silk crests
...
export default function SoftAurora({
  ...
  // 1. Initial WebGL Canvas & Render Loop Setup (Mounted once)
  useEffect(() => {
  ...
  // 2. Dynamic Uniform Updates (Silky-smooth prop updates with ZERO canvas re-creation or flashing)
  useEffect(() => {
